...
import LocalMarketCartData from "../../../components/cart/LocalMarketCartData";
...
import { clearLocalMarketCart } from "../../../redux/store/slices/LocalMarketCartSlice";
import { calculateItemTotal } from "../../../utils/calculateItemTotal";
import { toast } from "../../../utils/toast";

const LocalMarketCartTab = () => {
    ...
    const destination = useMemo(() => ({
            latitude: localMarketCartItems[0]?.store_latitude,
            longitude: localMarketCartItems[0]?.store_longitude
        }), [localMarketCartItems[0]?.store_latitude, localMarketCartItems[0]?.store_longitude]
    );
    ...
    const itemsTotal = useMemo(() => {
        return localMarketCartItems.reduce((sum, item) => {
    ...
    const groupedByStore = localMarketCartItems.reduce((acc, item) => {
    ...
    const orderData = useMemo(() => {
        if (!localMarketCartItems.length) return null;

        return {
            user_id,
            order_type: 'Local Market',

            user_latitude: latitude ?? 0.0,
            user_longitude: longitude ?? 0.0,

            cart_qty: localMarketCartItems.length,
    ...
    const handleClearCart = () =>
        dispatch(clearLocalMarketCart(), toast.success("Cart cleared successfully"));
    ...
    const storeCount = new Set(localMarketCartItems.map(item => item.store_id)).size;

    const totalItems = localMarketCartItems.reduce(
        (sum, item) => sum + (item.product_qty || 0),
        0
    );

    const CartFooter = useMemo(() => {
        if (localMarketCartItems.length === 0) return null;
        return (
            <>
    ...
    }, [localMarketCartItems.length, origin, destination, delivery]);

    // Optimize SectionList rendering
    const renderItem = useCallback(({ item }) => <LocalMarketCartData item={item} />, []);
    ...
    return (
        <View className="flex-1 bg-white justify-center items-center">
            ...
            <SectionList
                ...
                ListHeaderComponent={() => (localMarketCartItems.length > 0 && (
                    <View className=''>
                        <View className='w-full mt-3 mb-5 items-center justify-start bg-gray-100 rounded p-2'>
                            <Text className='text-black text-base' style={{fontFamily: 'roboto-medium'}}>
                                You have {localMarketCartItems.length} item{localMarketCartItems.length !== 1 ? 's' : ''} from {storeCount} {storeCount > 1 ? 'different' : ''} store{storeCount !== 1 ? 's' : ''} in your local market cart.
                            </Text>
                        </View>
                    ...
                    <View style={{width: '100%', marginTop: 80}}
                        ...
                        <View className="flex-1 justify-center items-center relative">
                            <EmptyState
                                ...
                                description="Your local market cart is empty"
                            />
                        </View>
                        ...
                        <TouchableOpacity
                            ...
                            style={{width: '100%'}}
                            onPress={() => router.push('../local-market')}
                        >
...
export default LocalMarketCartTab;