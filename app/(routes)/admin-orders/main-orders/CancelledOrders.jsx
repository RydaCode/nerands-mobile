...
// --- Pending Orders ---
const CancelledOrders = ({ title, params }) => {
                ...
                <View className=" px-2 justify-center items-center my-6">
                    <View className="items-center py-[1px] justify-start px-2 w-full bg-[#F3F4F8]">
                        <Text className="text-black">
                            There {orderData?.length === 1 ? 'is': 'are'} <Text className='text-primary text-lg'>{orderData?.length}</Text> {title} {orderData?.length === 1 ? 'order': 'orders'} from this store
                        </Text>
                    </View>
                </View>
            )}
