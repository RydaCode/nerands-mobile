...
// --- Pending Orders ---
const AdminOrderHistory = ({ title, params }) => {
            ...
            ListHeaderComponent={() => (
                <View className=" px-2 justify-center items-center my-6">
                    <View className="items-center py-[1px] justify-start px-2 w-full bg-[#F3F4F8]">
                        <Text className="text-black" style={{fontFamily: 'roboto-medium'}}>
                            There {orderData?.length === 1 ? 'is': 'are'} <Text className='text-primary text-lg'>{orderData?.length}</Text> {orderData?.length === 1 ? 'Order': 'Orders'} in the {title} of this store
                        </Text>
                    </View>
                </View>
            )}
